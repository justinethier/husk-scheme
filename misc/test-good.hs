...
run env cont _ _  = do 
  ...
  (trace ("" ++ (show value) ++ " " ++ (show args)) continueEval) env (makeCPSWArgs env cont f479 []) result
defineFuncEntryPt480 env cont value (Just args) = do 
  value <- getRTVar env "foldl"
  (trace ("" ++ (show value) ++ " " ++ (show args)) continueEval) env (makeCPSWArgs env (makeCPSWArgs env cont applyNextArg482 $ args ++ [value] ++ []) applyFirstArg481 []) (Nil "") 
applyFirstArg481 env cont value (Just args) = do 
  value <- getRTVar env "mem-helper"
  (trace ("" ++ (show value) ++ " " ++ (show args)) continueEval) env (makeCPSWArgs env (makeCPSWArgs env cont applyNextArg484 $ args ++ [value] ++ []) applyFirstArg483 []) (Nil "") 

applyFirstArg483 :: Env -> LispVal -> LispVal -> Maybe [LispVal] -> IOThrowsError LispVal 
applyFirstArg483 env cont _ _  = do 
  (trace ("" ++ (show value) ++ " " ++ (show args)) continueEval) env (makeCPSWArgs env (makeCPSWArgs env cont applyNextF486 []) applyStubF485 []) $ Nil""

applyStubF485 :: Env -> LispVal -> LispVal -> Maybe [LispVal] -> IOThrowsError LispVal 
applyStubF485 env cont _ _  = do 
  val <- getRTVar env "curry"
  (trace ("" ++ (show value) ++ " " ++ (show args)) continueEval) env cont val
...
applyNextArg484 env cont value (Just args) = do 
  ...
  (trace ("" ++ (show value) ++ " " ++ (show args)) continueEval) env (makeCPSWArgs env (makeCPSWArgs env cont applyWrapper $ args ++ [value] ++ []) applyFirstArg487 []) (Nil "") 
...
applyNextArg482 env cont value (Just args) = do 
  ...
  (trace ("" ++ (show value) ++ " " ++ (show args)) continueEval) env (makeCPSWArgs env (makeCPSWArgs env cont applyNextArg489 $ args ++ [value] ++ []) applyFirstArg488 []) (Nil "") 
...
applyFirstArg488 env cont value _  = do 
  ...
  (trace ("" ++ (show value) ++ " " ++ (show args)) continueEval) env cont x1
applyNextArg489 env cont value (Just args) = do 

  (trace ("" ++ (show value) ++ " " ++ (show args)) continueEval) env (makeCPSWArgs env (makeCPSWArgs env cont applyWrapper $ args ++ [value] ++ []) applyFirstArg490 []) (Nil "") 
...
f479 env cont value (Just args) = do 
  ...
  (trace ("" ++ (show value) ++ " " ++ (show args)) continueEval) env (makeCPSWArgs env (makeCPSWArgs env cont applyNextArg492 $ args ++ [value] ++ []) applyFirstArg491 []) (Nil "") 
...
applyNextArg492 env cont value (Just args) = do 
  ...
  (trace ("" ++ (show value) ++ " " ++ (show args)) continueEval) env (makeCPSWArgs env (makeCPSWArgs env cont applyWrapper $ args ++ [value] ++ []) applyFirstArg493 []) (Nil "") 
