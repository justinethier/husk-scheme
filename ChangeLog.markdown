...
TODO: before releasing:
 - try to get something working WRT libraries and the compiler. Even if it is a hack, the important thing is to assess how feasible this is going forward.
 - possibly fix cond/case begin issue, but it can wait for the next release or two if necessary
 - possibly see if there are any more compiler enhancements that can be made in a reasonable amount of time, such as reducing the amount of compiled code or fixing/triaging TODO's
 - anything else?
...
- Significantly reduced the amount of compiled Haskell code generated by huskc. This lead to a 40% reduction in the code generated by the compiler test suite, although any actual percentages depend upon the scheme code being compiled.
...
- Added compiler optimizations to evaluate primitive expressions at compile time, and to generate more efficient code for functions that are only passed literal arguments.
